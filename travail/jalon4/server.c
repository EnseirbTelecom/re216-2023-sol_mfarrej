...
// ============================ Fonctions de gestion de la liste chaînée des clients connectés ============================
...
// ============================ Fonctions de gestion de la liste chaînée des salons ============================
...
// ============================ Fonctions de gestion au niveau de la socket d'écoute  ============================
...
// Cette fonction permet de gérer l'envoi d'un flag à un client pour le notifier del'acceptation de sa demande de connexion
// Elle retourne 0 en cas de succès et -1 en cas d'erreur
...
int handle_receive(int client_socket, struct message* received_structure, char** payload) {
	...
	if (received_structure->type == NICKNAME_LIST
		|| received_structure->type == MULTICAST_LIST
		|| received_structure->type == FILE_REJECT)
		return 0;
...
// ============================ Fonctions de gestion des demandes utilisateur ============================
...
int handle_broadcast_request(Client** head, int sender_socket, struct message response_struct, char* payload) {
   	...
   	char payload_to_send[strlen(sender_pseudo)+15+INFOS_LEN]; 
   	strcpy(payload_to_send, "[");
   	strcat(payload_to_send, sender_pseudo);
   	strcat(payload_to_send, ">Everyone] : ");
   	strcat(payload_to_send, payload);
   	
   	response_struct.pld_len = strlen(sender_pseudo)+15+strlen(payload);
...
// Cette fonction permet de vérifier l'existence d'un client identifié par son pseudonyme. Au cas où le client existe, elle renseigne aussi un pointeur vers sa structure via l'argument client_structure.
// Elle retourne 1 si le client existe, et 0 sinon.
int check_client_exists(Client** head, char* client_pseudo,
						Client** client_structure){
	// Parcours de la liste chaînée pour trouver le client
    Client* current = *head;
    while (current != NULL) {
        if (strcmp(current->pseudo, client_pseudo) == 0){
        	*client_structure = current;
        	return 1;
        }
        current = current->next;
    }
...
int handle_PM_request(Client** head, char* received_info, int sender_socket, struct message response_struct) {
	...
	Client* client_structure = NULL;
	int exists = check_client_exists(head, target_pseudo, &client_structure);
    ...
    if (exists){
		...
		int ret = handle_send(client_structure->fd, response_struct, payload_to_send);
		if (ret == -1)
			return -1;
...
// Cette fonction permet de gérer le transfert de la demande d'autorisation d'envoi d'un fichier à l'utilisateur destinataire (avec la commande /send, type FILE_REQUEST). Elle notifie le client à l'origine de la demande du succès du transfert de la demande ou de son échec par l'envoi de flags. 
// La fonction retourne 0 en cas de succès et -1 en cas d'erreur.  
int handle_file_send_request(Client** head, int sender_socket, struct message response_struct, char* file_path){
	
	char receiver_pseudo[NICK_LEN];
	char file_name[FILE_NAME_LEN];
	// Récupération du pseudo du client à l'origine et du nom du fichier à envoyer (uniquement le dernier élément du chemin, avec la fonction standard basename())
	strcpy(receiver_pseudo, response_struct.infos);
	strcpy(file_name, basename(file_path));
	
	// Vérification que le destinataire existe
	Client* receiver_structure = NULL;
	int exists = check_client_exists(head, receiver_pseudo, &receiver_structure);
	
	if (exists){
		// Transfert au destinataire de la demande d'envoi de fichier
    	char request_string[strlen(response_struct.nick_sender)+strlen(" wants you to accept the transfer of the file named \"\".\n")+strlen(file_name)+strlen("Do you accept? [Y/N]")]; 
   		strcpy(request_string, response_struct.nick_sender);
   		strcat(request_string, " wants you to accept the transfer of the file named \"");
   		strcat(request_string, file_name);
   		strcat(request_string, "\".\n");
   		strcat(request_string, "Do you accept? [Y/N]");
   			
   		response_struct.pld_len = strlen(request_string);
   			
		int ret = handle_send(receiver_structure->fd, response_struct, request_string);
		if (ret == -1)
			return -1;
			
		// Notification du client à l'origine de l'envoi que le transfert a bien été fait: Envoi du flag "0"
        response_struct.pld_len = strlen("0");
        ret = handle_send(sender_socket, response_struct, "0");
        if (ret == -1)
			return -1;
		...
		// Cas où le client n'existe pas : Envoi du flag "1"
  		response_struct.pld_len = strlen("1");
  		strcpy(response_struct.infos, "0");		
		int ret = handle_send(sender_socket, response_struct, "1");
		if (ret == -1)
			return -1;
...
// Cette fonction permet de gérer l'approbation ou le rejet d'un demande d'envoi d'un fichier (types FILE_ACCEPT et FILE_REJECT). En cas d'pprobation, le serveur transfert l'adresse et le port sur lequel le récepteur écoutera la connexion de l’émetteur. En cas de rejet, le serveur notifie l'émetteur par un message approprié.
// La fonction retourne 0 en cas de succès et -1 en cas d'erreur.  
int handle_file_send_response(Client** head, struct message response_struct, char* payload){
	
	// Récupération du pseudo de l'émetteur de la demande d'envoi de fichier
	char sender_pseudo[NICK_LEN];
	strcpy(sender_pseudo, response_struct.infos);
	
	// Sachant que l'émetter existe, on utlise la fonction check_client_exists pour touver la structure de l'émetteur
	Client* sender_structure = NULL;
	check_client_exists(head, sender_pseudo, &sender_structure);
	int sender_socket = sender_structure->fd;
	
	// Adaptation du message à envoyer à l'émetteur selon la décision du destinataire
    switch (response_struct.type) {
        
        case FILE_ACCEPT:
            int ret = handle_send(sender_socket, response_struct, payload);
        	if (ret == -1)
				return -1;
            break;

        case FILE_REJECT:
            char response_string[INFOS_LEN];
    		strcpy(response_string, "[Server] : ");
    		strcat(response_string, response_struct.nick_sender);
    		strcat(response_string, " cancelled the file transfer.");
    
			response_struct.pld_len = strlen(response_string);		
			strcpy(response_struct.infos, response_string);
			ret = handle_send(sender_socket, response_struct, response_string);
			if (ret == -1)
				return -1;
            break;
    }
...
// Cette fonction permet de gérer l'acquittement de réception d'un fichier par le récepteur (type FILE_ACK).
// Le payload recu par le serveur contiendra le pseudonyme de l'utilisateur à l'origine de l'envoi du fichier.  
// Nomenclature : "sender" fait référence à l'utilisateur à l'origine de la demande d'envoi de fichier, "receiver" fait référence à l'utilisateur destinataire de l'envoi du fichier 
// La fonction retourne 0 en cas de succès et -1 en cas d'erreur.  
int handle_file_send_ack(Client** head, struct message response_struct, char* payload){
	
	// Récupération du pseudo du récepteur du fichier
	char receiver_pseudo[NICK_LEN];
	strcpy(receiver_pseudo, response_struct.nick_sender);
	// Récupération du pseudo de l'émetteur du fichier
	char sender_pseudo[NICK_LEN];
	strcpy(sender_pseudo, payload);
	
	// Sachant que l'émetteur existe, on utlise la fonction check_client_exists pour touver la structure de l'émetteur
	Client* sender_structure = NULL;
	check_client_exists(head, sender_pseudo, &sender_structure);
	int sender_socket = sender_structure->fd;
    
	// Envoi du message d'acquittement à l'émetteur    
    char ack_string[INFOS_LEN];
   	strcpy(ack_string, "[Server] : ");
    strcat(ack_string, receiver_pseudo);
    strcat(ack_string, " has received the file \"");
    strcat(ack_string, response_struct.infos);
    strcat(ack_string, "\".");
    
	response_struct.pld_len = strlen(ack_string);		
	strcpy(response_struct.infos, ack_string);
	int ret = handle_send(sender_socket, response_struct, ack_string);
	if (ret == -1)
		return -1;
...
int handle_user_request(Client** head_clients,  Channel** head_channels, struct pollfd* fds, int fds_idx, int nb_channels, struct message received_structure, char* payload_data){

	int client_fd = fds[fds_idx].fd;
	...
	switch (request_type)
	{
		...
		case NICKNAME_NEW :
			ret = handle_nickname_request(head_clients, payload_data, client_fd, received_structure);
			if (ret == -1)
				return -1;
			break;
		// Cas d'une demande de la liste des utilisateurs connectés
		case NICKNAME_LIST :
			ret = handle_list_request(head_clients, client_fd, received_structure);
			if (ret == -1)
				return -1;
			break;
		// Cas d'une demande d'informations sur un utilisateur
		case NICKNAME_INFOS :
			ret = handle_info_request(head_clients, payload_data, client_fd, received_structure);
			if (ret == -1)
				return -1;
			break;
		// Cas d'une demande d'envoi d'un message en diffusion
		case BROADCAST_SEND :
			ret = handle_broadcast_request(head_clients, client_fd, received_structure, payload_data);
			if (ret == -1)
				return -1;
			break;
		// Cas d'une demande d'envoi d'un message privé
		case UNICAST_SEND :
			ret = handle_PM_request(head_clients, payload_data, client_fd, received_structure);			
			if (ret == -1)
				return -1;
			break;
		...
		case ECHO_SEND :
			...
			if (strcmp(payload_data, "/quit") == 0 || strcmp(payload_data, "/quit ") == 0){
				...
				remove_Client(head_clients, client_fd);
				close(client_fd); 
				fds[fds_idx].fd = 0;
				break;
			}							
			...
			// Renvoi du message
			int ret = handle_send(client_fd, received_structure, echo_buffer);
			if (ret == -1)
				return -1;
			break;
		// Cas d'une demande de création de salon	
		case MULTICAST_CREATE :
			ret = handle_create_request(head_channels, head_clients, nb_channels, payload_data, client_fd, received_structure);
			if (ret == -1)
				return -1;
			else if (ret == -2)
				return -2;
		...
		case MULTICAST_LIST :
			ret = handle_channel_list_request(head_channels, client_fd, received_structure);
			if (ret == -1)
				return -1;		
			break;
		// Cas d'une demande de rejoindre un salon
		case MULTICAST_JOIN :
			ret = handle_join_request(head_channels, payload_data, head_clients, client_fd, received_structure);
			if (ret == -1)
				return -1;
			break;
		// Cas d'une demanade d'envoi de message dans un salon	
		case MULTICAST_SEND :
			ret = handle_channel_send_request(head_channels, client_fd, received_structure, payload_data); 
			if (ret == -1)
				return -1;
			break;
		// Cas d'une demande de sortie d'un salon 	
		case MULTICAST_QUIT :
			ret = handle_quit_request(head_channels, head_clients, payload_data, client_fd, received_structure);
			if (ret == -1)
				return -1;
			break;
		// Cas d'une demande d'autorisation d'envoi de fichier
		case FILE_REQUEST :
			ret = handle_file_send_request(head_clients, client_fd, received_structure, payload_data);
			if (ret == -1)
				return -1;
			break;
		// Cas d'une approbation ou de rejet d'envoi de fichier
		case FILE_ACCEPT :
		case FILE_REJECT :
			ret = handle_file_send_response(head_clients, received_structure, payload_data);
			if (ret == -1)
				return -1;
			break;
		// Cas d'un acquittement de réception d'un fichier
		case FILE_ACK :
			ret = handle_file_send_ack(head_clients, received_structure, payload_data);
			if (ret == -1)
				return -1;
			break;
	}
	...
	if ( (fds[fds_idx].revents & POLLHUP) == POLLHUP ) {	
		remove_Client(head_clients, client_fd);
		close(client_fd); 
		fds[fds_idx].fd = 0;
...
int main(int argc, char* argv[]) {
	...
	int nb_channels = 0; // Nombre de salons existants
	...
	while(1){ 
		...
		if ( (fds[0].revents & POLLIN) == POLLIN ){				
			...
			add_Client(&head_clients, new_fd, client_addr, formatted_time);
		...
		for(int i = 1; i < MAX_CONN; i++){
			...
			if (fds[i].fd  != 0){
				// Traitement en cas de données à lire ou de déconnexion
				if( (fds[i].revents & POLLIN) == POLLIN || (fds[i].revents & POLLHUP) == POLLHUP) {
					...
					ret = handle_user_request(&head_clients, &head_channels, fds, i, nb_channels, received_structure, payload_data);
					// Contrôle d'erreur
					if (ret == -1){
						perror("write");
						free(payload_data);
